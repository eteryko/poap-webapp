...
const WordSchameWithActiveRequest = yup.object().shape({
  ...WordBaseShape,
  codesQuantity: yup
    .number()
    .required('the amount of requested codes must be greater or equals to zero')
...
const WordSchameWithoutActiveRequest = yup.object().shape({
  ...WordBaseShape,
  codesQuantity: yup
    .number()
    .required('A positive amount of codes is required')
...
export {
  ...
  PoapQrRequestSchema,
  WordSchameWithActiveRequest,
  WordSchameWithoutActiveRequest,
};
